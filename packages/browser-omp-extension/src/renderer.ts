...
export function formatFeedbackAsPrompt(event: BrowserFeedbackEvent): string {
	if (event.type === "dom.selection") {
		...
		const lines = [`Browser feedback from Chrome extension:`, `Page: ${event.page.url}`, `Element: ${elementRef}`];
		if (event.note) lines.push(`Note: "${event.note}"`);
		lines.push("", "Please apply this change.");
		return lines.join("\n");
	}
	if (event.type === "page.screenshot") {
		const lines = [`Browser screenshot feedback from Chrome extension:`, `Page: ${event.page.url}`];
		if (event.note) lines.push(`Note: "${event.note}"`);
...
function renderDomSelection(event: DomSelectionFeedback): string {
	const screenshot = event.screenshot ? `- Local reference: ${event.screenshot.ref}` : "- None";
	const accessibility = event.element.accessibility ? JSON.stringify(event.element.accessibility, null, 2) : "{}";
...
export function renderBrowserFeedbackContext(event: BrowserFeedbackEvent): string {
