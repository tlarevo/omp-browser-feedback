...
describe("generateSelector", () => {
	test("prefers a unique data-testid", () => {
		const { document } = parseHTML('<button data-testid="save">Save</button><button>Cancel</button>');
	...
	test("falls back to an nth-of-type path when stable attributes are unavailable", () => {
		...
		const element = document.querySelector("section:nth-of-type(2) > button:nth-of-type(2)");
		expect(element).toBeTruthy();

		expect(generateSelector(element as Element)).toBe("section:nth-of-type(2) > button:nth-of-type(2)");
	});
});
